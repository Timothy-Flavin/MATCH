...
class Agent_With_Oracle():
  def __init__(self, policy, value, n_agents, Q=None, sampler="Thompson"):
    ...
    for i in range(n_agents):
      if sampler == "Thompson":
        self.tberns.append(Thompson_Bernoulli(0.5,1.0,0.2))
      elif sampler == "UCB":
        self.tberns.append(UCB_Multinomial(2,0.5,0.1,1,0.1,0))
      else:
        self.tberns.append(EG_Multinomial(2,0.5,decay=0.95,prior_strength=1,experience_strength=0.2))
  ...
  def policy_with_oracle(self, state, oracle, oracle_num):
    leader = self.tberns[oracle_num].choose(0.5, active=np.array([1,1]))
    self.oracle_num = oracle_num
    chosen_action = 0
    if leader == 0:
      chosen_action = self.policy(state)
    else:
      chosen_action = oracle(state)
    return chosen_action, leader, self.tberns[oracle_num]
  
  def update_sampler(self, adv):
    self.tberns[self.oracle_num].update(adv)
...
def run_exp(policies, n_trials, n_agents, one_oracle=True, discount=0.99, verbose=0, update_sampler = True, n_shots=3, sampler = "Thompson"):
  ...
  for i in range(n_trials):
    ...
    while step<500 and not terminated:
      ...
      oracle_i = 0
      if not one_oracle:
        oracle_i = np.random.randint(0,n_agents)
      
      command = -1
      acp = np.copy(active)
      acp[oracle_i]=0
      if acp.sum() > 0:
        command = agents[oracle_i].choose_target(None, acp)
      if verbose>0:
      ...
      for a in range(n_agents):
        ...
        if a == command:
          action, leader, sampler = agents[a].policy_with_oracle(state, agents[oracle_i].policy, oracle_i)
          if update_sampler:
            agents[oracle_i].update_oracle(leader*2-1, verbose)
        else:
    ...
    if verbose>0 or i==n_shots-1:
      ...
      for a in agents:
        ...
        for ai in range(n_agents):
          print(f"    listen modes: {a.tberns[ai].dist_mode(None)}")
  return cum_rs
